...
var divider = function(nums, evens, odds){
    ...
    for(i=0; i< nums.length; i++){
        if(nums[i]%2){
            returnValue[1].push(nums[i]); 
        }
        else{
            returnValue[0].push(nums[i]); 
        }   
    }    
    return returnValue;
};
...
//then loop through the array to see if that random number is in the array. If it is, return true, if it's not, return false
  ...
  //Code Here
var finder = function(randomNum, numbers){
...
var removeItem = function(myGroceryList, remove){
    var i = myGroceryList.indexOf(remove);
        if(i !== -1){
